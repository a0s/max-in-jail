...
- **Централизованное хранение данных** - все данные в `~/.cache/max-in-jail/` для легкой очистки
...
```
max-in-jail/
├── max-in-jail.sh    # Основной скрипт
...
~/.cache/max-in-jail/       # Все данные скрипта (создается автоматически)
...
- **Легкая очистка**: Все данные хранятся в `~/.cache/max-in-jail/` - удалите эту папку для полной очистки
...
Все данные, созданные скриптом, хранятся в `~/.cache/max-in-jail/`. Для полной очистки:
